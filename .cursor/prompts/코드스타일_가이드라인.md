...
- 요청 객체는 항상 도메인별 단일 `Request` 클래스 사용 (예: `NoticeRequest`)
...
- 오류 코드는 도메인별로 그룹화하여 주석으로 구분 (예: `// COMMON`, `// GITHUB`, `// NOTICE` 등)
- 오류 코드 명명 규칙: `{도메인}_{오류유형}` 형식 (예: `GITHUB_REPOSITORY_NOT_FOUND`)
- 도메인별 오류 코드는 해당 도메인의 특성을 반영하여 최대한 상세하게 정의 (예: `GITHUB_ISSUE_URL_INVALID`, `GITHUB_REPOSITORY_ACCESS_DENIED` 등)
